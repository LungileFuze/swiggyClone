...
class UserRouter {
    ...
    getRoutes() {
        this.router.get('/send/verification/mail', GlobalMiddleWare.auth, UserController.resendVerificationEmail)
    ...
    postRoutes() {
        ...
        this.router.post('/logout', GlobalMiddleWare.auth, GlobalMiddleWare.decodeRefreshToken, GlobalMiddleWare.checkError, UserController.logout)
    }
    ...
    putRoutes() {
        this.router.patch('/update/profilePic', GlobalMiddleWare.auth, new Utils().multer.single('profileImages'), UserValidators.userProfilePic(), GlobalMiddleWare.checkError, UserController.updateUserProfilePic)
